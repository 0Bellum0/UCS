...
void inicializa_aeroporto(Aeroporto *aeroporto){
    ...
    aeroporto->fate.frente = aeroporto->fate.tras = NULL;
    aeroporto->fdec.frente = aeroporto->fdec.tras = NULL;
...
void enfileira(Fila *fila, Aviao *aviao){
    if(fila->tras == NULL){
        fila->frente = fila->tras = aviao;
    } else {
        fila->tras->prox = aviao;
        fila->tras = aviao;
    }
    aviao->prox = NULL;
}
